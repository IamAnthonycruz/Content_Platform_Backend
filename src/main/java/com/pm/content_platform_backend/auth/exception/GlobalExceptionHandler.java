...
import com.pm.content_platform_backend.auth.dto.ErrorResponseDTO;
import com.pm.content_platform_backend.auth.dto.FieldErrorDTO;
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponseDTO<Void>> handleInvalidRefreshTokenException(InvalidRefreshTokenException ex) {
        ErrorResponseDTO<Void> errorResponseDTO = new ErrorResponseDTO<>();
        errorResponseDTO.setStatus(HttpStatus.UNAUTHORIZED.value());
        errorResponseDTO.setMessage("Invalid refresh token");
        errorResponseDTO.setTimestamp(LocalDateTime.now());
        return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(errorResponseDTO);
    }
